...
Devise.setup do |config|
  ...
  # config.secret_key = Nenv.instance.devise_secret_key

  # ==> Controller configuration
  # Configure the parent class to the devise controllers.
  # config.parent_controller = 'Api::BaseApiController'
  ...
  # `from` parameter.
  config.mailer_sender = Nenv.instance.mailer_sender
  ...
  # `:mongoid` (bson_ext recommended) by default. Other ORMs may be available as
  # additional gems.
  ...
  # Tell if authentication through `request.params` is enabled. Default is
  # `true`. It can be set to an array that will enable params authentication
  ...
  # Tell if authentication through HTTP Auth is enabled. Default is `false`. It
  # can be set to an array that will enable http authentication only for the
  # given strategies, for example,
  #
  #     `config.http_authenticatable = %i[database]`
  #
  ...
  # If 401 status code should be returned for AJAX requests. Default is `true`.
  # config.http_authenticatable_on_xhr = true

  # The realm used in HTTP Basic Authentication. Default is `'Application'`.
  ...
  # sign-in and sign-up, you need to get a new CSRF token from the server. You
  ...
  config.pepper =
    if Rails.env.test? || Rails.env.development?
      Digest::MD5.hexdigest(Rails.application.class.name)
    else
      server_key = Nenv.instance.app_debug? ? :sandbox : :production

      Rails.application.credentials.dig(server_key, :devise_pepper)
    end
  ...
  # (default). You can use `:sha1`, `:sha512`, or algorithms from other
  # authentication tools as `:clearance_sha1`, `:authlogic_sha512` (then you
  # should set stretches above to `20` for default behavior), and
  # `:restful_authentication_sha1` (then you should set stretches to `10`, and
